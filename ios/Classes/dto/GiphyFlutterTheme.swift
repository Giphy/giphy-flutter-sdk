...
class GiphyFlutterTheme: GPHTheme {
    var config: [String: Any?]
    
    init(flutterConfig: [String: Any?]?) {
        config = deserializeFlutterConfig(flutterConfig)
        super.init()
...
private func deserializeFlutterConfig(_ rawConfig: [String: Any?]?) -> [String: Any?] {
    var config = rawConfig ?? [:]
    
    if let preset = config["preset"] as? String {
        config["preset"] = GPHThemeType.fromString(preset)
    }
    if let kbAppearance = config["keyboardAppearance"] as? String {
        config["keyboardAppearance"] = UIKeyboardAppearance.fromString(kbAppearance)
    }
    if let edScrollIndicatorStyle = config["emojiDrawerScrollIndicatorStyle"] as? String {
        config["emojiDrawerScrollIndicatorStyle"] = UIScrollView.IndicatorStyle.fromString(edScrollIndicatorStyle)
    }
    
    THEME_COLOR_FIELDS.forEach { field in
        if let color = config[field] as? UInt32 {
            config[field] = UIColor(argb: color)
        }
    }
